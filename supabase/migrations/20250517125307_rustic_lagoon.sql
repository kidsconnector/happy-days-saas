...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    AND policyname = 'Users can read their own tenant''s users'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    AND policyname = 'Children are viewable by authenticated users of the same tenant'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'children' 
    AND policyname = 'Children are insertable by authenticated users of the same tenant'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'children' 
    AND policyname = 'Children are updatable by authenticated users of the same tenant'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    AND policyname = 'Templates are viewable by authenticated users of the same tenant'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    AND policyname = 'Campaigns are viewable by authenticated users of the same tenant'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    AND policyname = 'API keys are viewable by authenticated users of the same tenant'
  ) THEN
